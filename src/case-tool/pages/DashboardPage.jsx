...
function ProjectCard({ project, onClick }) {
  ...
  const progress = project.features.length > 0
    ...
    : 0
  const progressColor = progress >= 100 ? '#22C55E' : progress >= 50 ? '#F59E0B' : '#3776A1'
  const projectStats = [
    { label: 'Features', value: project.features.length, color: '#003A6B', bg: '#EAF4FB', border: '#B8D7EC' },
    { label: 'Estimations', value: project.estimations.length, color: '#1D6FA5', bg: '#EDF7FD', border: '#C8E4F6' },
    { label: 'Risks', value: project.risks.length, color: highRisks > 0 ? '#D14343' : '#5293BB', bg: highRisks > 0 ? '#FDECEC' : '#F1F8FD', border: highRisks > 0 ? '#F6C7C7' : '#D2E8F5' },
  ]
