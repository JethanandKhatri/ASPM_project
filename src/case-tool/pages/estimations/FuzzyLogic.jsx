...
import { HeaderIconShell, IconCheck, IconFuzzy } from './EstimationIcons'
    ...
    <div style={{ padding: 28 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20 }}>
              ...
              <tbody>
                {project.features.map((f, i) => (
                  <tr key={f.id} style={{ borderBottom: `1px solid ${C.border}`, background: i % 2 === 0 ? C.cardBg : C.mainBg }}>
                    ...
                    <td style={{ padding: '10px' }}>
                      <select
                        value={classifications[f.id] || 'medium'}
                        onChange={e => setClassifications(c => ({ ...c, [f.id]: e.target.value }))}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {saved ? (
            <div style={{ padding: 14, background: C.success + '12', border: `1px solid ${C.success}30`, borderRadius: 8, textAlign: 'center', fontSize: 13, color: C.success, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
